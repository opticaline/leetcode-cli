...
plugin.getSubmission = function(submission, cb) {
  ...
  request(opts, function(e, resp, body) {
    ...
    if (re) submission.runtimeDistributionFormatted = JSON.parse(eval(re[1]));
    re = body.match(/memoryDistributionFormatted:\s('[^']+')/);
    if (re) submission.memoryDistributionFormatted = JSON.parse(eval(re[1]));
    re = body.match(/runtime:\s'([^']+)'/);
    if (re) submission.runtime = re[1];
    re = body.match(/memory:\s'([^']+)'/);
    if (re) submission.memory = re[1];
